...
canopy uses **Flux-First with Emergency Override** deployment strategy. This means:
...
This compares `canopy/k8s/` with `pi-fleet/clusters/eldertree/canopy/`
...
- `canopy/k8s/` - Direct deployment manifests
- `pi-fleet/clusters/eldertree/canopy/` - Flux-managed manifests

**After any changes**:

```bash
# Copy changes to pi-fleet
cp k8s/*.yaml ../pi-fleet/clusters/eldertree/canopy/

# Validate sync
./scripts/validate-k8s-sync.sh

# Commit both locations
cd ../pi-fleet && git add . && git commit -m "Update canopy manifests"
cd ../canopy && git add . && git commit -m "Update k8s manifests"
```
...
kubectl logs -n canopy -l app=canopy-backend
kubectl logs -n canopy -l app=canopy-frontend
```
...
# Manual sync
diff -r k8s/ ../pi-fleet/clusters/eldertree/canopy/
```
