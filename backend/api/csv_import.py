...
async def get_format_config(bank_format: BankFormat):
    ...
    if bank_format in csv_service.BANK_FORMATS:
        ...
        return {"bank_format": bank_format.value, "field_mapping": mapping.model_dump()}
...
async def preview_csv_import(
    ...
    try:
        preview = csv_service.parse_csv_file(file_content=file_content, config=config, existing_transactions=[])
    except Exception as e:
    ...
    with get_db_session() as db:
        for tx_create in transactions_to_create:
            try:
                ...
                errors.append({"description": tx_create.description, "error": str(e)})
...
async def get_import_history(limit: int = Query(50, ge=1, le=100), offset: int = Query(0, ge=0)):
    ...
    with get_db_session() as db:
        ...
        # Get total count
        total_query = select(func.count(func.distinct(TransactionModel.import_id))).where(
            TransactionModel.import_id.isnot(None)
        )
...
async def delete_import(import_id: str):
    ...
    with get_db_session() as db:
        ...
        count_query = select(func.count(TransactionModel.id)).where(TransactionModel.import_id == import_id)
        ...
        # Delete transactions
        delete_query = select(TransactionModel).where(TransactionModel.import_id == import_id)
