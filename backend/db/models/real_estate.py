...
class RealEstateProperty(Base):
    ...
    ownership_percentage: Mapped[Decimal] = mapped_column(
        Numeric(precision=5, scale=4), default=Decimal("1.0")
    )  # 0.5 = 50% ownership
    ...
    # Value estimation
    estimated_market_value: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    value_estimated_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    monthly_rent: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    ...
    payment_series: Mapped[list["RealEstatePaymentSeries"]] = relationship(
        "RealEstatePaymentSeries", back_populates="property", cascade="all, delete-orphan"
    )
    payments: Mapped[list["RealEstatePayment"]] = relationship(
        "RealEstatePayment", back_populates="real_estate_property", cascade="all, delete-orphan"
    )
    ...
    def total_paid(self) -> Decimal:
        ...
        return sum(
            (p.amount_paid or Decimal("0")) * self.ownership_percentage for p in self.payments if p.status == "paid"
        )
...
class RealEstatePaymentSeries(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    property_id: Mapped[int] = mapped_column(ForeignKey("real_estate_properties.id", ondelete="CASCADE"))
    ...
    nominal_amount_per_installment: Mapped[Decimal] = mapped_column(Numeric(precision=18, scale=2))
    ...
    # Relationships
    property: Mapped["RealEstateProperty"] = relationship("RealEstateProperty", back_populates="payment_series")
...
class RealEstatePayment(Base):
    ...
    series_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("real_estate_payment_series.id", ondelete="SET NULL"), nullable=True
    )
    ...
    # Relationships
    real_estate_property: Mapped["RealEstateProperty"] = relationship("RealEstateProperty", back_populates="payments")
