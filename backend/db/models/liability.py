...
class Liability(Base):
    ...
    liability_type: Mapped[str] = mapped_column(String(50), default="credit_card")
    ...
    # Balance tracking
    current_balance: Mapped[Decimal] = mapped_column(Numeric(precision=18, scale=2), default=Decimal("0"))
    ...
    credit_limit: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    available_credit: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    ...
    promotional_apr: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=6, scale=4), nullable=True)
    promotional_apr_end_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
    ...
    # Payment details
    minimum_payment: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    fixed_payment: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(precision=18, scale=2), nullable=True
    )  # For loans with fixed monthly payments
    payment_due_day: Mapped[Optional[int]] = mapped_column(nullable=True)  # Day of month (1-31)
    next_payment_due: Mapped[Optional[date]] = mapped_column(Date, nullable=True)

    # Statement details
    statement_closing_day: Mapped[Optional[int]] = mapped_column(nullable=True)  # Day of month statement closes
    last_statement_balance: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    ...
    annual_fee: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    ...
    balance_history: Mapped[list["LiabilityBalanceHistory"]] = relationship(
        "LiabilityBalanceHistory", back_populates="liability", cascade="all, delete-orphan"
    )
...
class LiabilityBalanceHistory(Base):
    ...
    # Relationships
    liability: Mapped["Liability"] = relationship("Liability", back_populates="balance_history")
...
class LiabilityPayment(Base):
    ...
    # Breakdown (for loans)
    principal_amount: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    interest_amount: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=18, scale=2), nullable=True)
    ...
    # Relationships
    liability: Mapped["Liability"] = relationship("Liability", back_populates="payments")
